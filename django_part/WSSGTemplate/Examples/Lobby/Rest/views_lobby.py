...
def get_lobby_by_id(request):
    ...
    model_instance = views.generic_get_by_id(Lobby, name=id_req)
...
def get_lobbies(request):
    return views.get_all_with_communication(request, Lobby)
...
def set_lobby_by_id(request):
    ...
    id_req = result_from_check["id"]
    # get current lobby
    current_lobby = utilities.search_object_by_attribute(Lobby, lobbyID=id_req)
    ...
    if "players" in extracted_values_key:
        ...
        for index, player_id in enumerate(player_names_req):
            player_instance = utilities.search_object_by_attribute(Player, genericID=player_id)
...
def create_lobby(request):
    ...
    player = utilities.search_object_by_attribute(Player, name=host_req)
...
def join_lobby(request):
    ...
    player = utilities.search_object_by_attribute(Player, name=id_req)
    if player is None:
        return utilities.server_message_response(message="player was not found")
