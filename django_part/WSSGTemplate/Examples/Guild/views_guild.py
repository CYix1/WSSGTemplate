...
def get_guild_by_id(request):
    return views.get_by_id_with_communication(request, Guild)
...
def get_guilds(request):
    return views.get_all_with_communication(request, Guild)
...
def set_guild_by_id(request):
    ...
    current_guild = utilities.search_object_by_attribute(Guild, guildName=guild_name_req)
    ...
    if "guildPlayers" in extracted_values_keys:
        ...
        for index, player_name in enumerate(player_names):
            ...
            player_instance = utilities.search_object_by_attribute(Player, name=player_name)
    ...
    if "guildOwner" in extracted_values_keys:
        ...
        guild_owner = utilities.search_object_by_attribute(Player, name=guild_owner_name_req)
