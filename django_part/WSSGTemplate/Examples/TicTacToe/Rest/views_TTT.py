...
def surrender_game(request):
    ...
    lobby = utilities.search_object_by_attribute(Lobby, lobbyID=lobby_id_req)
    if lobby is not None:

        game = utilities.search_object_by_attribute(TTTGame, lobby__lobbyID=lobby.lobbyID)
        if game is not None:
            game.delete()
        lobby.delete()
...
def end_game(request):
    ...
    if player_won_req == "DRAW":
        ...
        player_won = utilities.search_object_by_attribute(Player, name=player_won_req)
...
def create_TTTGame(request):
    ...
    host = utilities.search_object_by_attribute(Player, name=host_req)
    if host is None:
        return utilities.server_message_response(message="the host player could not be found")
    opponent = utilities.search_object_by_attribute(Player, name=opponent_req)
    if opponent is None:
        return utilities.server_message_response(message="the opponent player could not be found")

    game = utilities.search_object_by_attribute(TTTGame, lobby__lobbyID=lobby_id_req)
    if game is None:
...
def get_game(request):
    ...
    current_lobby = utilities.search_object_by_attribute(Lobby, lobbyID=lobby_id_req)
    ...
    if current_lobby.winner is not None:
        return utilities.server_message_response(message=f"Player {current_lobby.winner.name} won", status=200,
                                                 identifier="MESSAGE")
    current_game = utilities.search_object_by_attribute(TTTGame, lobby__lobbyID=lobby_id_req)
    if current_game is None:
