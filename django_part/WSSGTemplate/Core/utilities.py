...
def search_object_by_attribute(model: Type[models.Model], **kwargs):
    try:
        return model.objects.get(**kwargs)
    except ObjectDoesNotExist:
        print(f"Object of model {model.__name__} with {kwargs} does not exist.")
        return None
    except Exception as e:
        print(f"An error occurred while retrieving {model.__name__} with {kwargs}: {e}")
        return None
...
def get_readable_lobby_json(instance):
    ...
    result["players"] = convert_ids_to_list_of_names(result["players"], Player)
        ...
        if isinstance(obj, Model):
            ...
            for field in obj._meta.get_fields():
                if field.is_relation and field.many_to_one:
                    # Handle foreign key relation
                    related_obj = getattr(obj, field.name)
                    if related_obj is not None:
                        data[field.name] = self.default(related_obj)
...
def nested_to_json_string(obj):
    return json.dumps(obj, cls=CustomDjangoJSONEncoder, indent=4)
