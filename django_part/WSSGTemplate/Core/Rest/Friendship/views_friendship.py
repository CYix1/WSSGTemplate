...
def add_friend(request):
    ...
    player = utilities.search_object_by_attribute(Player, name=player_name)
    # Basic check if available
    if player is None:
        return utilities.server_message_response(
            message=f"player {player_name} was not found")

    friend = utilities.search_object_by_attribute(Player, name=friend_name)
    if friend is None:
        return utilities.server_message_response(
...
def create_friend_request(request):
    ...
    # get both players
    player_requestor = utilities.search_object_by_attribute(Player, name=player_name_req)
    if player_requestor is None:
        return utilities.server_message_response(
            message=f"no player was found")
    friend_receiver = utilities.search_object_by_attribute(Player, name=friend_name_req)
