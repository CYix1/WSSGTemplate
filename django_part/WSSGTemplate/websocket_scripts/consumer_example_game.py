...
from WSSGTemplate.models import TTTGame, Lobby
...
class TTTWSGame(BasicWSServerLayer):
    ...
    async def handle_create(self, data):
        ...
        lobby = await utilities.asearch_object_by_attribute(ServerClass.Lobby, lobbyID=data["lobbyId"])
        if lobby is None:
            await self.send_message_to_client(identifier="ERROR", message="lobby somehow not found")
            return

        host = await utilities.asearch_object_by_attribute(ServerClass.Player, name=data["host"])
        if host is None:
            await self.send_message_to_client(identifier="ERROR", message="the host player could not be found")
            return

        opponent = await utilities.asearch_object_by_attribute(ServerClass.Player, name=data["opponent"])
        if opponent is None:
            await self.send_message_to_client(identifier="ERROR", message="the opponent player could not be found")
            return

        game = await  utilities.asearch_object_by_attribute(ServerClass.TTTGame, lobby__lobbyID=data["lobbyId"])
        if game is None:
    ...
    async def handle_update_game(self, data):
        ...
        current_game = await utilities.asearch_object_by_attribute(ServerClass.TTTGame, lobby__lobbyID=data["lobbyId"])
    ...
    async def handle_end_game(self, data):
        ...
        if data["playerWon"] == "DRAW":
            ...
            lobby.winner = await utilities.asearch_object_by_attribute(ServerClass.Player, name=data["playerWon"])
        lobby.isActive = False
        lobby.save()

        game = await utilities.asearch_object_by_attribute(ServerClass.TTTGame, lobby__lobbyID=lobby.lobbyID)
        if game is not None:
            game.delete()
        if data["playerWon"] == "DRAW":
